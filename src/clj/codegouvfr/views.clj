...
(defn default [lang & [title subtitle content]]
  (let [title    (or title (i/i lang [:index-title]))
    ...
    (h/html5
     ...
     _paq.push([\"setCookieDomain\", \"*.code.etalab.gouv.fr\"]);
     _paq.push([\"setDomains\", [\"*.code.etalab.gouv.fr\"]]);
     ...
     (function() {
      ...
      [:noscript [:p [:img {:src   "//stats.data.gouv.fr/piwik.php?idsite=95&amp;rec=1"
                            :style "border:0;"
                            :alt   ""}]]]]
     [:body
      ...
      [:nav {:class "navbar" :role "navigation" :aria-label "main navigation"}
       [:div {:class "navbar-brand"}
        [:a {:class "navbar-item" :href "https://code.etalab.gouv.fr"}
